            ...
            if (primaryGrabbingObject && secondaryGrabbingObject)
            {
                switch (secondaryGrabAction)
                {
                    case SecondaryControllerActions.Control_Direction:
                        FixedUpdateSecondaryControlDirection(primaryGrabbingObject.transform, secondaryGrabbingObject.transform);
                        break;
                }
            }
        }

        private void FixedUpdateSecondaryControlDirection(Transform primaryGrabbingObject, Transform secondaryGrabbingObject)
        {
            /*
            Quaternion rotationDelta = Quaternion.LookRotation(secondaryGrabbingObject.position);

            float angle;
            Vector3 axis;
            rotationDelta.ToAngleAxis(out angle, out axis);

            transform.RotateAround(primaryGrabbingObject.position, axis, angle);
            */
            //transform.rotation = Quaternion.LookRotation(secondaryGrabbingObject.transform.position, primaryGrabbingObject.transform.TransformDirection(Vector3.up));
            //transform.LookAt(secondaryGrabbingObject);
            //transform.Translate(primaryGrabbingObject.position - originalControllerAttachPoint.position, Space.World);
